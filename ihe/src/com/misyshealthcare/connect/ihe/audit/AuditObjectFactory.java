...
import com.misyshealthcare.connect.ihe.audit.jaxb.ActiveParticipantType;
import com.misyshealthcare.connect.ihe.audit.jaxb.AuditMessage;
import com.misyshealthcare.connect.ihe.audit.jaxb.AuditSourceIdentificationType;
import com.misyshealthcare.connect.ihe.audit.jaxb.CodedValueType;
import com.misyshealthcare.connect.ihe.audit.jaxb.EventIdentificationType;
import com.misyshealthcare.connect.ihe.audit.jaxb.ObjectFactory;
import com.misyshealthcare.connect.ihe.audit.jaxb.ParticipantObjectIdentificationType;
import com.misyshealthcare.connect.ihe.audit.jaxb.TypeValuePairType;
...
public class AuditObjectFactory {
	...
	private Identifier dAA; // Default Asigning Authority
	private Identifier dMA; // Default Misys Authority
	private String     dIE; // Default Id Encoding
	private String     dNE; // Default NameEncoding
	private String     dDT; // Default document type.
	private IMessageTransmitter messenger;
	
	/** Generates and initalizes a blank audit message factory. */
	public AuditObjectFactory(IMessageTransmitter messenger) {
		...
		dAA = messenger.getAuditTrailDescription().getAsigningAuthority();
		dMA = messenger.getAuditTrailDescription().getLocalAsigningAuthority();
		dIE  = messenger.getAuditTrailDescription().getIdEncoding();
		dNE  = messenger.getAuditTrailDescription().getNameEncoding();
		dDT = messenger.getAuditTrailDescription().getDocumentTypeCode();
	}
		...
		throws JAXBException
	{
		...
		JAXBContext messageContext = JAXBContext.newInstance("com.misyshealthcare.connect.ihe.audit.jaxb");
		...
		throws JAXBException 
	{
		...
		participant.setParticipantObjectID(participantObject.getId(dAA, dMA, dIE));
		...
		if (dDT != null && participantObject.documentDesc != null) {
			TypeValuePairType tvpt = messageFactory.createTypeValuePairType();
			tvpt.setType(dDT);
			BASE64Encoder encoder = new BASE64Encoder();
			String b64Desc = encoder.encode(participantObject.documentDesc.getBytes());
			tvpt.setValue(b64Desc.getBytes());
			participant.getParticipantObjectDetail().add(tvpt);
		}
		if (participantObject.sensitivity != null)
			participant.setParticipantObjectSensitivity(participantObject.sensitivity);
		...
		if (participantObject.query != null) {
			BASE64Encoder encoder = new BASE64Encoder();
			String b64Query = encoder.encode(participantObject.query.getBytes());
			participant.setParticipantObjectQuery(b64Query.getBytes());
		}		
