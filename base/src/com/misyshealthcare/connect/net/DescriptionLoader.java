 ...
 * 
 * @author Jim Firby
 * @version 2.0 - Nov 8, 2005
 */
public class DescriptionLoader {

	/* Logger for debugging messages */
	private static final Logger LOG = Logger.getLogger(DescriptionLoader.class);
	 ...
	 */
	public static List<IConnectionDescription> loadConnectionDescriptions(String filename) throws SAXException, IOException, ParserConfigurationException {
	 ...
	 * @throws ParserConfigurationException When no XML parser can be found
	 */
	private static void addConnectionDescriptions(List<IConnectionDescription> descriptions, File file) throws SAXException, IOException, ParserConfigurationException {
		...
		for (int elementIndex = 0; elementIndex < allDescriptionElements.getLength(); elementIndex++) {
			...
			if (name.equalsIgnoreCase("SECURECONNECTION") || name.equalsIgnoreCase("STANDARDCONNECTION")) {
				...
				if (filename == null) filename = getNodeAsText(currentDescriptionElement);
				if (filename != null) {
					try {
	...
	private static void processDescription(StandardConnectionDescription description, NodeList descriptionNodes, File parent) {
		...
		for (int valueIndex = 0; valueIndex < descriptionNodes.getLength(); valueIndex++) {
			...
			if (!handled) handled = handleSecureConnectionValue(itemName, item, description, parent);
			...
			if (!handled) {
				if (itemName.equalsIgnoreCase("CODETYPE")) {
					...
					if (filename == null) filename = getNodeAsText(item);
					if (filename != null) {
					...
					if (propertyValue == null) propertyValue = getNodeAsText(item);
	 ...
	 * 
	 * @param description The connection description object being described
	 * @param filename The included configuration file describing that connection
	 ...
	 * @throws ParserConfigurationException When no XML parser can be found
	 */
	public static boolean processDescriptionIncludeFile(StandardConnectionDescription description, InputStream stream, File parent) throws SAXException, IOException, ParserConfigurationException {
		// Create a builder factory and a builder, and get the document.
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		factory.setValidating(false);
	...
	private static boolean handleSecureConnectionValue(String itemName, Node item, StandardConnectionDescription description, File parent) {
		...
		if (itemName.equalsIgnoreCase("KEYSTORE")) {
			...
			if (filename != null) {
				...
				if (secure != null) secure.setKeyStore(keyFile.getAbsolutePath());
				else LOG.error("KEYSTORE element in non secure connection.");
			} else {
				logMissingAttributeWarning(itemName, "name", description);
			}
		} else if (itemName.equalsIgnoreCase("KEYPASS")) {
			if (secure != null) secure.setKeyStorePassword(getNodeAsText(item));
			else LOG.error("KEYSTORE element in non secure connection.");
		} else if (itemName.equalsIgnoreCase("TRUSTSTORE")) {
			String filename = getNodeAsText(item);
			if (filename != null) {
				File keyFile = new File(parent.getParentFile(), filename);
				if (secure != null) secure.setTrustStore(keyFile.getAbsolutePath());
				else LOG.error("KEYSTORE element in non secure connection.");
			} else {
				logMissingAttributeWarning(itemName, "name", description);
			}
		} else if (itemName.equalsIgnoreCase("TRUSTPASS")) {
			if (secure != null) secure.setTrustStorePassword(getNodeAsText(item));
			else LOG.error("KEYSTORE element in non secure connection.");
		} else {
			handled = false;
		}
	 ...
	 * @param codingScheme The DOM element of the "CodeType" entry
	 */
			...
			for (int i=0; i<codes.getLength(); i++) {
				...
				if (codeNode.getNodeName().equalsIgnoreCase("CODE")) {
					...
					if (code == null) {
						...
						if (display == null) {
							if (classScheme != null) logMissingAttributeWarning(codeNode.getNodeName(), "display", connection);
							display = code;
						} 
	 ...
	 * @param propertySet The DOM element of the "AssigningAuthority" entry
	 */
	...
	private static void logUnexpectedTagWarning(String tagName, IConnectionDescription description) {
		String name = null;
		if (description != null) name = description.getName();
